...
**Cierre de la migración (25 ago 2026):** `src/app/icon.tsx` y `src/app/opengraph-image.tsx` quedaron fuera de la primera ronda a propósito (cargan fuentes a mano vía `next/og`, no vía `next/font`), pero se migraron en un segundo paso — ambos usan Oswald ahora (700 el nombre/monograma, 600 el eyebrow y la tagline de la OG image; ninguno de los dos tiene texto de cuerpo real, así que Newsreader no hacía falta ahí). **Cero archivos del repo referencian ya Space Grotesk, IBM Plex Sans o IBM Plex Mono** (verificado por grep sobre `src/` completo) — la migración tipográfica está cerrada sin excepciones pendientes.
...
**Favicon (24 ago 2026, migrado a Oswald 25 ago 2026):** `src/app/icon.tsx`, generado en build con `next/og` (mismo patrón que `opengraph-image.tsx`, `dynamic = "force-static"`), reemplaza el `favicon.ico` default de `create-next-app` (borrado). Monograma "C" (no "CF" — probado, dos letras se leen como mancha a tamaño real de pestaña) en Oswald 700, blanco sobre `--accent`. Legibilidad verificada con el downscale real a 16×16 vía canvas (mismo suavizado que usa un navegador), no solo en la vista previa grande — re-verificada tras el cambio de fuente.
