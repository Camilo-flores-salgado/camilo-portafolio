...
- [x] **Migración tipográfica completa (25 ago 2026): Space Grotesk + IBM Plex Sans + IBM Plex Mono → Oswald + Newsreader (2 familias, no 3).** Hecha en 4 fases, un cambio a la vez:
  ...
  - `src/app/icon.tsx` y `opengraph-image.tsx` quedaron fuera a propósito en esta ronda (cargan fuentes a mano vía `next/og`, no vía `next/font`) — migrados en un segundo paso, ver siguiente ítem.
...
- [x] **Cierre de la migración — `icon.tsx` y `opengraph-image.tsx` (25 ago 2026).** Los dos archivos que quedaron fuera a propósito de la migración anterior (fuentes cargadas a mano vía `next/og`, no `next/font`) se migraron: favicon → Oswald 700 (antes Space Grotesk), legibilidad a 16×16 re-verificada tras el cambio. OG image → Oswald 700 (nombre) / 600 (eyebrow + tagline); sin Newsreader porque ninguno de los tres textos es cuerpo real. `npm run build` limpio, ambas rutas (`/icon`, `/opengraph-image`) generan PNG válidos en el export estático, first-load JS sin cambio (112,1 KB — son imágenes de build, no JS de cliente). **Cero archivos en el repo referencian ya Space Grotesk/IBM Plex** (grep sobre `src/` completo) — CLAUDE.md §6 actualizado confirmándolo, sin excepciones pendientes.
